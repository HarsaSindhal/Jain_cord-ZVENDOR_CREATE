...
WHERE BusinessPartnerIDByExtSystem = @taxkid INTO @DATA(BPPARTNER).
 ...
 DATA(ROLL) = zmdm_module_cus_ven_roll_api=>GETDATA( customer = BPPARTNER bproll = 'FLVN01' ).
...
IF VENDORDETAIL-a-BusinessPartner <> ''.
 ...
 DATA(RELESTIONSHIP) = zmdm_module_relationship_api=>GETDATA( VENDOR = VENDORDETAIL-a-BusinessPartner taxkid = taxkid  ).
