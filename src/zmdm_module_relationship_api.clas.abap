CLASS zmdm_module_relationship_api DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZMDM_MODULE_RELATIONSHIP_API IMPLEMENTATION.
...
SELECT SINGLE * FROM zrelationshiptab_CDS WITH PRIVILEGED ACCESS as a WHERE
Taskid = @taxkid INTO @Data(company).
 ...
 DATA(lb) = '}'.
  ...
  fin_json = fin_json &&
  |     { '{' }   | &
  |"BusinessPartner1": "{ vendor }",                      | &
  | "BusinessPartner2": "{ company-Businesspartner }",   | &
  | "ValidityEndDate": "{ company-Validto+0(4) }-{ company-Validto+4(2) }-{ company-Validto+6(2) }T00:00:00",   | &
  | "ValidityStartDate": "{ company-Validfrom+0(4) }-{ company-Validfrom+4(2) }-{ company-Validfrom+6(2) }T00:00:00",   | &
  | "IsStandardRelationship": true,   | &
  | "RelationshipCategory": "{ company-Relationshipcatalog+1(6) }",   | &
  | "BPRelationshipType": ""   | &
  |     { '}' }    | .
 ...
 CONDENSE fin_json .
   ...
   TRY .
    DATA(status261) = Vendo_po( json = fin_json vendor = vendor  ).
...
METHOD get_mat.
  ...
  ENDMETHOD.
   ...
   DATA(RESS) = GETDATA( vendor = '0100000200'  taxkid = '2000030'  ).
  ENDMETHOD.
...
LV_URL = |https://{ cl_abap_context_info=>get_system_url(  ) }:443/sap/opu/odata/sap/API_BUSINESS_PARTNER/A_BusinessPartner('{ vendor }')/to_BPRelationship| .
